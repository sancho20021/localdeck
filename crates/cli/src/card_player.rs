...
pub fn run_card_player(storage: &mut Storage, output: Output) {
    let (qr_events, scanner) = start_qr_scanner();

    let (audio_errors, player) = match start_music_player(output) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("Failed to start music player: {e}");
            return;
        }
    };
            ...
            recv(audio_errors) -> msg => {
                match msg {
                    Ok(e) => {
                        ...
                        if matches!(e, AudioPlayerError::UnsupportedDevice(..)) {
                            eprintln!("Device can't play audio, terminating");
                            scanner.shutdown();
                            player.shutdown();
                            return;
                        }
